...
def __init__(self):
    # ✅ ИСПРАВЛЕНО: Удалена строка self.claude = ClaudeClient()
    ...
    # ✅ ИСПРАВЛЕНО: Удален параметр claude_client из всех парсеров
    # Initialize parsers
...
async def _parse_document(
    ...
    try:
        ...
        if settings.FALLBACK_ENABLED:
            ...
            try:
                # ✅ ИСПРАВЛЕНО: Создаем Claude локально для fallback
                claude = ClaudeClient()
...
async def _audit_position(
    ...
    try:
        ...
        # ✅ ИСПРАВЛЕНО: Создаем Claude локально для audit
        claude = ClaudeClient()
...
1. Zkontroluj popis a jednotku
1. Zkontroluj cenu (pokud je uvedena)
1. Zkontroluj množství (pokud je uvedeno)
1. Identifikuj případné problémy
...
async def _extract_project_context(
    ...
    if drawings_data and settings.ANTHROPIC_API_KEY:
        try:
            # ✅ ИСПРАВЛЕНО: Создаем Claude локально
            claude = ClaudeClient()
