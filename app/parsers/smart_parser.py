...
class SmartParser:
    ...
    def parse_excel(self, file_path: Path) -> Dict[str, Any]:
        ...
        logger.info(f"📊 Excel file: {file_path.name} ({size_mb:.1f}MB)")
        
        if size_mb < SIZE_THRESHOLD_MB:
            # Небольшой файл → стандартный парсер
            logger.info("✅ Using standard pandas parser")
            try:
                return self.excel_parser.parse(file_path)
            except Exception as e:
                logger.warning(f"Standard parser failed: {e}")
                logger.info("⚠️ Falling back to streaming parser")
                return self.streaming_excel.parse(file_path)
        else:
            # Большой файл → streaming парсер
            logger.info("✅ Using streaming parser (large file)")
            return self.streaming_excel.parse(file_path)
    ...
    def parse_pdf(self, file_path: Path) -> Dict[str, Any]:
        ...
        logger.info(f"📄 PDF file: {file_path.name} ({size_mb:.1f}MB)")
    ...
    def parse_xml(self, file_path: Path) -> Dict[str, Any]:
        ...
        logger.info(f"📝 XML file: {file_path.name} ({size_mb:.1f}MB)")
        
        if size_mb < SIZE_THRESHOLD_MB:
            # Небольшой файл → стандартный KROS парсер
            logger.info("✅ Using standard KROS parser")
            try:
                return self.kros_parser.parse(file_path)
            except Exception as e:
                logger.warning(f"Standard parser failed: {e}")
                logger.info("⚠️ Falling back to streaming parser")
